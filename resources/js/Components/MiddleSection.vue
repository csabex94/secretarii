<template>
    <div class="bg-white w-full h-full flex flex-col justify-start items-stretch border-r-2 border-l-2 border-gray-100 lg:rounded-r-md xl:rounded-none">
        <!-- Header with name -->
        <div v-if="$page.props.user.current_team" class="flex flex-row items-center justify-between px-3 py-2 bg-gray-50 bg-opacity-40 border-b-2 border-gray-100">
            <div class="">
                <h2 class="font-medium">{{ $page.props.user.current_team.name }}</h2>
                <p class="text-xs text-gray-500">{{ $page.props.user.current_team.users.length }} memebres</p>
            </div>
            <div class="flex flex-row">
                ...
                <button @click="openShowAddTopicForm" type="button" class="p-2 ml-2 text-gray-400 rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring" aria-label="Search">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                </button>
                <button type="button" class="p-2 ml-2 text-gray-400  rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring" aria-label="Open">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-6-3a2 2 0 11-4 0 2 2 0 014 0zm-2 4a5 5 0 00-4.546 2.916A5.986 5.986 0 0010 16a5.986 5.986 0 004.546-2.084A5 5 0 0010 11z" clip-rule="evenodd" />
                    </svg>
                </button>
                <button @click="handleOpenRightSide" type="button" class="p-2 ml-2 text-gray-400 rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring" aria-label="More">
...
<!--        <messages />-->
        ...
        <div v-if="showAddTopicForm" class="flex flex-row justify-between items-center p-3">
            <div class="flex-1 px-3">
                <input type="text" class="w-full border-2 border-gray-100 rounded-full px-4 py-1 outline-none text-gray-500 focus:outline-none focus:ring" placeholder="Add new topic">
            </div>
        </div>
<!--        <div v-if="showAddTopicForm" class="flex flex-row justify-between items-center p-3">-->
<!--            <div class="">-->
